...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Trouver les surfaces encadrantes pour l'interpolation
    ...
    if (sprinklers && !partialSprinklers) {
        ...
        // Pour la distance limitative inférieure
        let lowerSurfacePercentageLower = tableToUse[lowerSurface][lowerDistanceIndex];
        let upperSurfacePercentageLower = tableToUse[upperSurface][lowerDistanceIndex];
        ...
        let upperSurfacePercentageUpper = tableToUse[upperSurface][upperDistanceIndex];
        ...
        // Pour la distance limitative inférieure
        let lowerSurfacePercentageLower = tableToUse[lowerSurface][ratioCategory][lowerDistanceIndex];
        let upperSurfacePercentageLower = tableToUse[upperSurface][ratioCategory][lowerDistanceIndex];
    ...
    let resultHTML = `
        ...
        <strong>1. Pour chaque borne de distance, interpolation par surface:</strong><br>
    `;

    // Ajouter les détails spécifiques selon le type de bâtiment (avec/sans gicleurs)
    if (sprinklers && !partialSprinklers) {
        resultHTML += `
            - À ${lowerDistance} m: interpolation entre ${lowerSurface} m² (${tableToUse[lowerSurface][lowerDistanceIndex]}%) 
            et ${upperSurface} m² (${tableToUse[upperSurface][lowerDistanceIndex]}%)<br>
            - Pourcentage à ${lowerDistance} m pour une surface de ${facadeSurface.toFixed(2)} m²: ${percentageLowerDistance.toFixed(2)}%<br>
            - À ${upperDistance} m: interpolation entre ${lowerSurface} m² (${tableToUse[lowerSurface][upperDistanceIndex]}%) 
            et ${upperSurface} m² (${tableToUse[upperSurface][upperDistanceIndex]}%)<br>
            - Pourcentage à ${upperDistance} m pour une surface de ${facadeSurface.toFixed(2)} m²: ${percentageUpperDistance.toFixed(2)}%<br>
        `;
    } else {
        resultHTML += `
            - À ${lowerDistance} m: interpolation entre ${lowerSurface} m² (${tableToUse[lowerSurface][ratioCategory][lowerDistanceIndex]}%) 
            et ${upperSurface} m² (${tableToUse[upperSurface][ratioCategory][lowerDistanceIndex]}%)<br>
            - Pourcentage à ${lowerDistance} m pour une surface de ${facadeSurface.toFixed(2)} m²: ${percentageLowerDistance.toFixed(2)}%<br>
            - À ${upperDistance} m: interpolation entre ${lowerSurface} m² (${tableToUse[lowerSurface][ratioCategory][upperDistanceIndex]}%) 
            et ${upperSurface} m² (${tableToUse[upperSurface][ratioCategory][upperDistanceIndex]}%)<br>
            - Pourcentage à ${upperDistance} m pour une surface de ${facadeSurface.toFixed(2)} m²: ${percentageUpperDistance.toFixed(2)}%<br>
        `;
    }

    resultHTML += `
        <strong>2. Interpolation finale par distance limitative:</strong><br>
        - Entre ${lowerDistance} m (${percentageLowerDistance.toFixed(2)}%) et ${upperDistance} m (${percentageUpperDistance.toFixed(2)}%)<br>
        - Formule: ${percentageLowerDistance.toFixed(2)} + (${limitingDistance.toFixed(2)} - ${lowerDistance}) / (${upperDistance} - ${lowerDistance}) × (${percentageUpperDistance.toFixed(2)} - ${percentageLowerDistance.toFixed(2)})<br>
    ...
    // NOUVELLE MÉTHODE D'INTERPOLATION
    // D'abord interpoler par surface pour chaque distance, puis par distance
    ...
    let percentageLowerDistance;
    if (upperSurface === ">100") {
        // Pour les très grandes surfaces, utiliser directement le pourcentage de la plus grande surface définie
        percentageLowerDistance = upperSurfacePercentageLower;
    } else {
        percentageLowerDistance = lowerSurfacePercentageLower + 
    ...
    if (upperSurface === ">100") {
        // Pour les très grandes surfaces, utiliser directement le pourcentage de la plus grande surface définie
        percentageUpperDistance = upperSurfacePercentageUpper;
    } else {
        percentageUpperDistance = lowerSurfacePercentageUpper + (surface - lowerSurface) / (upperSurface - lowerSurface) * (upperSurfacePercentageUpper - lowerSurfacePercentageUpper);
    }
    
    // Interpolation finale par distance limitative
    let finalPercentage = percentageLowerDistance + (limitingDistance - lowerDistance) / (upperDistance - lowerDistance) * (percentageUpperDistance - percentageLowerDistance);
    
    // Appliquer la formule pour les grandes surfaces si nécessaire
    ...
    // [Code existant pour déterminer les exigences, vérifications et afficher les résultats]
    ...
    let resultHTML = `
         ...
         <br><strong>Détails d'interpolation par la méthode exacte:</strong><br>
         <strong>1. Pour chaque borne de distance, interpolation par surface:</strong><br>
         - À ${lowerDistance} m: interpolation entre ${lowerSurface} m² (${lowerSurfacePercentageLower}%) 
            et ${upperSurface !== ">100" ? upperSurface : 100} m² (${upperSurfacePercentageLower}%)<br>
         - Pourcentage à ${lowerDistance} m pour une surface de ${surface.toFixed(2)} m²: ${percentageLowerDistance.toFixed(2)}%<br>
         - À ${upperDistance} m: interpolation entre ${lowerSurface} m² (${lowerSurfacePercentageUpper}%) 
            et ${upperSurface !== ">100" ? upperSurface : 100} m² (${upperSurfacePercentageUpper}%)<br>
         - Pourcentage à ${upperDistance} m pour une surface de ${surface.toFixed(2)} m²: ${percentageUpperDistance.toFixed(2)}%<br>
         <strong>2. Interpolation finale par distance limitative:</strong><br>
         - Entre ${lowerDistance} m (${percentageLowerDistance.toFixed(2)}%) et ${upperDistance} m (${percentageUpperDistance.toFixed(2)}%)<br>
         - Formule: ${percentageLowerDistance.toFixed(2)} + (${limitingDistance.toFixed(2)} - ${lowerDistance}) / (${upperDistance} - ${lowerDistance}) × (${percentageUpperDistance.toFixed(2)} - ${percentageLowerDistance.toFixed(2)})<br>
         <br><strong>Résultats :</strong><br>
   ...
   function calculate91015() {
    ...
    // Appliquer la formule pour les grandes surfaces si nécessaire
    if (surface > 100 && limitingDistance >= 1.2) {
        // Pour les habitations, la formule est DL²
        finalPercentage = Math.pow(limitingDistance, 2);
        // Limiter à 100%
        finalPercentage = Math.min(finalPercentage, 100);
    }
