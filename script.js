...
function formatCNBCalculationSteps() {
    ...
    const extrapolationDL = (distanceInferieure === null || distanceSuperieure === null || adjustedDistance < distancesUtilisees[0] || adjustedDistance > distancesUtilisees[distancesUtilisees.length - 1]);
    const extrapolationSurface = (surfaceInferieure === null || surfaceSuperieure === null || facadeSurface < surfacesUtilisees[0] || facadeSurface > surfacesUtilisees[surfacesUtilisees.length - 1]);
    const isExtrapolation = extrapolationDL || extrapolationSurface;

    if (isExtrapolation) {
        let extrapolationText = output + "\n\n⚠️ Calcul par extrapolation :\n";
        extrapolationText += "À défaut de données encadrantes disponibles dans le tableau, le calcul a été effectué par extrapolation linéaire à partir des valeurs extrêmes.\n";

        if (extrapolationDL) {
            extrapolationText += `DL utilisée : ${adjustedDistance.toFixed(2)} m (hors bornes du tableau)\n`;
        }

        if (extrapolationSurface) {
            extrapolationText += `Surface utilisée : ${facadeSurface.toFixed(2)} m² (hors bornes du tableau)\n`;
        }

        extrapolationText += `Résultat estimé : ${resultatFinal.toFixed(2)} %\n`;
        return extrapolationText;
    }
...
function format91014CalculationSteps() {
    ...
    const extrapolationDL = (distanceInferieure === null || distanceSuperieure === null || adjustedDistance < distancesUtilisees[0] || adjustedDistance > distancesUtilisees[distancesUtilisees.length - 1]);
    const extrapolationSurface = (surfaceInferieure === null || surfaceSuperieure === null || surface < surfacesUtilisees[0] || surface > surfacesUtilisees[surfacesUtilisees.length - 1]);
    const isExtrapolation = extrapolationDL || extrapolationSurface;
    ...
    if (isExtrapolation) {
        ...
        if (extrapolationSurface) {
            extrapolationText += `Surface utilisée : ${surface.toFixed(2)} m² (hors bornes du tableau)\n`;
        }
